d3.queue()
  ...
  .defer(d3.csv, "file:///Users/mdakbar/Desktop/Chaity/d3final/data/all_data.csv", function(row) {
    return {
  ...
  .await(function(error, mapData, data) {
    ...
    d3.selectAll('input[name="data-type"]')
        .on("change", () => {
          ...
          drawMap(geoData, data, currentYear, currentDataType);
          drawMap( data, currentDataType, country);
        });
...
function drawMap(geoData, climateData, year, dataType) {
  ...
  geoData.forEach(d => {
    var countries = climateData.filter(row => d.id === row.countryCode);
    var name = '';
